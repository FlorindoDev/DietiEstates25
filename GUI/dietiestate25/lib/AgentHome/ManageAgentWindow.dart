...
class ManageAgentWindow extends StatefulWidget {

  const ManageAgentWindow({super.key, required this.appbar});

  final AppBar appbar;

  @override
  _ManageAgentWindowState createState() => _ManageAgentWindowState();
}

class _ManageAgentWindowState extends State<ManageAgentWindow> {
  
  late Future<List<AgenteImmobiliare>> agenti;
  ...
  void reload() {
    setState(() {
      agenti = AgentHomeController.getAgenti(context, "");
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Container(
        ...
        child:
        FutureBuilder<List<AgenteImmobiliare>>(
        future: agenti,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            List<AgenteImmobiliare> agenti = snapshot.data!;
            return ListView.builder(
              controller: _scrollController,
              itemCount: agenti.length,
              itemBuilder: (context, index) {
                AgenteImmobiliare agente = agenti[index];
                ...
                return Container(
                  ...
                  ListTile(
                    ...
                    leading: Image(
                      image: MemoryImage(base64Decode(agente.immagineprofile ?? 
                      ...
                      children:[
                        SubmenuButton(
                          menuChildren: [
                            MenuItemButton(
                              onPressed: () async {
                                await AgentHomeController.removeAgent(context, agente, "");
                                reload();
                               },
                              child: const Text('Elimina'),
                            ),
                            MenuItemButton(
                              //onPressed: (agente.issupportoammi == true) ? () async { 
                                //await AgentHomeController.promoteAdmin(context, admin, "");
                                //reload();
                              //} : null,
                            ...
                            MenuItemButton(
                              //onPressed: (admin.issupportoammi == false) ? () async { 
                                //await AgentHomeController.unPromoteAdmin(context, admin, "");
                               // reload();
                              //} : null,
